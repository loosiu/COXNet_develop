...
from mmdet.core import (anchor_inside_flags, bbox_overlaps, build_assigner,
                        ...
                        reduce_mean, unmap, build_bbox_coder)
...
class GFLQHead(AnchorHead):
    ...
    def __init__(self,
        ...
        self.qls_bbox_coder = build_bbox_coder(qls_bbox_coder)
    ...
    def forward_single(self, x, scale):
        ...
        centerness = self.gfl_centerness(reg_feat)
    ...
    def loss_single(self, anchors, cls_score, bbox_pred, centerness, labels, label_weights,
        ...
        return loss_cls, loss_bbox, loss_dfl, loss_centerness, weight_targets.sum()
    ...
    def centerness_target(self, anchors, gts):
        ...
        elif self.centerness == 2:
            # 结合高斯和权重距离
            distance = left_right.sum(dim=-1) + top_bottom.sum(dim=-1)
            min_distance = left_right.min(dim=-1)[0] + top_bottom.min(dim=-1)[0]
            centerness = min_distance / distance

            anchors_centers = torch.stack([anchors_cx, anchors_cy], dim=1)
            gts_centers = torch.stack([(gts[:, 0] + gts[:, 2]) / 2, (gts[:, 1] + gts[:, 3]) / 2], dim=1)
            dist_squared = torch.sum((anchors_centers - gts_centers) ** 2, dim=1)
            gaussian_weight = torch.exp(-dist_squared / (2 * self.gaussian_sigma ** 2))

            combined_centerness = centerness * gaussian_weight
            combined_centerness = torch.clamp(combined_centerness, min=0, max=1)
            assert not torch.isnan(combined_centerness).any()
            return combined_centerness
    ...
    def _get_target_single(self,
        ...
        if self.use_pred:
            de_bbox_preds = self.integral(bbox_preds)
            dec_bbox_preds = self.qls_bbox_coder.decode(anchors, de_bbox_preds)
        else:
            cls_scores = None
            dec_bbox_preds =None
