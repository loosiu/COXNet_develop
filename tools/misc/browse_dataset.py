...
def parse_args():
    ...
    parser.add_argument('--config', 
                        default='checkpoints/vis_comparison/coxnet_star_vis.py',
                        # default='process_configs/niicu/ablation/atss_dual.py',
    ...
    parser.add_argument(
        '--output-dir',
        default='work_dir/1/',
        # default='work_dir/niicu/vis_results/visulization_fp/gt/',
        type=str,
...
def main():
    ...
    for item in dataset:
        filename_v = os.path.join(args.output_dir, 'vis',
                                Path(item['filename'][0]).name
                                ) if args.output_dir is not None else None
        filename_t = os.path.join(args.output_dir, 'thermal',
                                Path(item['filename'][0]).name
                                ) if args.output_dir is not None else None
